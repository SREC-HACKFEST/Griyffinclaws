package com.example.teacherapp1.automtl.common;
...
public class CameraSource {
    ...
    private AutoMLImageLabelerProcessor frameProcessor;
    ...
    public Size getPreviewSize() {
        Log.d(TAG, "getPreviewSize: previewSize: " + previewSize);
        return previewSize;
    }
    ...
    private Camera createCamera() throws IOException {
        ...
        if (requestedCameraId == -1) {
            Log.d(TAG, "createCamera: Could not find requested camera");
        ...
        if (sizePair == null) {
            Log.d(TAG, "createCamera: Could not find suitable preview size");
            throw new IOException("Could not find suitable preview size.");
        }

        Size pictureSize = sizePair.pictureSize();

        Log.d(TAG, "createCamera: pictureSize" + pictureSize.getHeight());
        ...
        if (pictureSize != null) {
            ...
            Log.d(TAG, "createCamera: not null");
    ...
    public void setMachineLearningFrameProcessor(AutoMLImageLabelerProcessor processor) {
        synchronized (processorLock) {
            cleanScreen();
            if (frameProcessor != null) {
                frameProcessor.stop();
